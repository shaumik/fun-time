...
const TARGETS = [
  ...
  /* The only target that is not a single file: Bridge reads a config JSON
     sitting beside index.html. Both go in the zip. */
  { dir:'playgama',         provider:'playgama',         host:'bridge.playgama.com',
    note:'~22 platforms; replace playgama-bridge-config.json from their editor',
    extra:'playgama-bridge-config.json' },
];
...
for (const t of TARGETS) {
  ...
  /* A placeholder config, so the build is runnable and the shape is obvious.
     The real one comes out of Playgama's config editor with the actual game
     id and ad placements in it — this is not it, and says so. */
  if (t.extra === 'playgama-bridge-config.json') {
    fs.writeFileSync(path.join(dir, t.extra), JSON.stringify({
      _README: 'PLACEHOLDER. Replace with the file generated by Playgama\'s config editor at developer.playgama.com — this one has no real game id and will not serve ads.',
      gameId: 'REPLACE_ME',
      platforms: {},
      advertisement: { placementFallback: 'default' }
    }, null, 2) + '\n');
  }
}
