...
function districtCfg(n, type, act){
  ...
  return {
    ...
    /* boss roads run a third longer: the fight is banking under pressure,
       and "the unit got away" was 4 of 23 measured run-endings */
    len: Math.round((400 + n * 26) * (boss ? 1.3 : 1)),
...
function bossDamage(amount){
  const b = G.boss;
  if (!b) return;
  b.hp -= amount * (Tree.has('t_exec') ? 1.25 : 1);
  b.hitFlash = 1;
...
function stepBoss(dt){
  ...
  if (b.def.id === 'warden') {
    ...
    if (b.atk <= 0) {
      b.atk = 8.5; G.pulseWarn = 0;
      /* only punishes hoarding — bank little and often and it does nothing.
         The threshold scales with the fight: a fixed 2,400 was tuned for
         act-1 numbers, and by act 2 two wrecks at mult 4 tripped it. */
      if (G.pending > Math.max(2400, Math.round(b.maxHp * 0.2))) {
...
function drawGround(){
  ...
  /* sparse city blocks in the void: a deterministic hash drops a dim slab in
     roughly a third of cells, so the off-road ground reads as a city seen
     from above rather than graph paper */
  if (QF.city) {
...
function syncHUD(){
  ...
  if (cfg) {
    ...
    if (bossing) {
      ...
      UI.objLbl.textContent = run.overtime ? 'Overtime ×1.5 XP' : 'Quota';
      UI.objVal.textContent = run.overtime
        ? fmt(run.banked) + ' banked'
        : fmt(Math.min(run.banked, run.quota)) + ' / ' + fmt(run.quota);
