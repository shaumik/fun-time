...
const Ads = {
  ...
  /* A build compiled with no ad provider at all — the standalone/itch target.
     Distinct from "no provider yet": off a local file or the playable page
     there is no provider either, and there simulating a placement is exactly
     what you want. Only the build can tell the two apart, so it says. */
...
/* ---- camera shake ----
   ...
   dizzy — and the per-frame resampling is as much to blame as the size, since
   noise at frame rate reads as a strobe rather than as an impact. The camera
   now trims the amplitude and swings it as two slow cosines at unrelated
   frequencies: the hit still lands, but as a rumble the eye can track.
...
function step(dt){
  ...
  /* a fresh hit restarts the waveform, so the kick lands on the frame of the
     impact rather than wherever the oscillator happened to be */
  ...
  cam.sx = Math.cos(G.shakeT * 24.0) * amp;
  cam.sy = Math.cos(G.shakeT * 17.3) * amp * 0.8;
