...
function drawTrail(v, mul, rich){
  ...
  /* Nested constant-width bands approximate a lateral falloff, and how many
     you use is how soft the shoulder is. Three left a visible step where the
     alpha jumped from 0.13 to 0.57 — a hard line running the length of the
     ribbon. Four ramps it 0.06 / 0.16 / 0.54 / core, which reads as a glow
     rather than as stacked rectangles. Traffic keeps three: at the size those
     ribbons occupy the extra fill buys nothing. */
  const passes = rich
    ? [[2.60, col,       0.06 * mul * drive * hot],
       [1.75, col,       0.10 * mul * drive * hot],
       [1.12, col,       0.38 * mul * drive * hot],
       [0.30, '#FFFFFF', 0.34 * mul * drive * hot * (v.boost ? 1.5 : 1)]]
    : [[1.55, col,       0.14 * mul * drive * hot],
       [1.00, col,       0.32 * mul * drive * hot],
       [0.32, '#FFFFFF', 0.26 * mul * drive * hot]];
  ...
  for (const [wm, c, al] of passes) {
    ...
    g.addColorStop(0.55, hexA(c, a * 0.62));
