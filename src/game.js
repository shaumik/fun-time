...
function districtCfg(n, type, act){
  ...
  const cfg = {
    ...
    /* ---- sized against what a pass actually pays ----
       A pass of three pays 532 and a pass of four pays 999, so 2,600 was
       three good lines — the fight was over before the unit finished
       announcing itself, which is exactly the "I won and I don't know how"
       report. At 5,200 it is ten to twelve passes, or most of a boss road. */
    bossHp: Math.round(5200 * Math.pow(1.5, Math.max(0, act - 1))),
...
/* ---- the centre line must not be a free harvest ----
   The inner lanes sat at +/-0.21, which on a 290-unit half-road is 61px from
   the middle — against a 48px contact radius and 12px of jitter, a car
   driving dead straight down the centre clips both inner lanes continuously.
   Measured, the do-nothing bot wrecked 20-69 cars a run and survived as long
   as the bot solving for the best line: reading the road was worth nothing
   because not reading it harvested anyway.

   Pulled apart so the middle of the road is a genuine gap. Now every car you
   take is a lane you chose to be in, which is the entire premise. */
const LANES = [-0.70, -0.34, 0.34, 0.70];
...
/* ============================================================
   ...
   ============================================================ */
const PURSUIT = [
  /* ---- speeds are set against the player's 620, and sit ABOVE it ----
     These were dropped under 620 to make the chase escapable. That was the
     wrong lever and it broke the game: THE PLAYER HAS NO THROTTLE. The car
     runs at full power always, so it is at 620 on every straight, and a
     pursuit that tops out at 592 can never close — measured, you never even
     saw them. "Outrun them" was never a verb the player had.

     They are faster than you again, so they always close in the end. The
     escape is the thing the whole game is built on: leave wreckage and let
     them drive into it. Two hits and a chaser is scrap, and a kill buys real
     quiet on the spawn clock. That is an answer the player performs, not one
     the speedometer hands them. */
  { id:'interceptor', name:'Interceptor', w:30, minAct:1,
    hp:3, len:52, wid:24, nose:.60, tail:.72, top:656, power:600, grip:7.0, turn:1,
    /* the baseline: closes and rams. Dodge it, or ram it back with a boost. */
    role:'Closes and rams.' },

  { id:'bull', name:'Bull', w:16, minAct:1,
    hp:4, len:62, wid:30, nose:.95, tail:.80, top:624, power:660, grip:6.2, turn:0.72,
  ...
  { id:'blocker', name:'Blocker', w:16, minAct:1,
    ...
    hp:3, len:66, wid:30, nose:.92, tail:.94, top:700, power:640, grip:7.4, turn:0.9,
    boxy:true,
  ...
  { id:'outrider', name:'Outrider', w:18, minAct:1,
    hp:1, len:34, wid:14, nose:.42, tail:.52, top:724, power:520, grip:8.6, turn:1.5,
  ...
  { id:'spotter', name:'Spotter', w:12, minAct:2,
    hp:2, len:54, wid:25, nose:.55, tail:.85, top:640, power:560, grip:7.2, turn:1,
...
function stepBoss(dt){
  ...
  if (b.breakT > 0) {
    ...
    if (b.def.id === 'warden') {
      ...
      if (b.atk <= 0) {
        b.atk = 6.5 * fuse; G.pulseWarn = 0;
        /* ---- the pulse attacks your weapon ----
           It used to wipe a pending bank, and banking no longer exists, so
           after the rebuild it reset your pass if you happened to be holding
           three cars and did nothing otherwise — a slap, on the boss that
           already had the least presence. The ceiling it computed was dead
           code that nothing read.

           Wreckage is the only thing that hurts anything in this game, so
           that is what the signal takes: the pulse detonates every piece of
           debris on the road, clearing the barricade you were building and
           killing the line you were on. You answer it by timing your passes
           between pulses, which is a real thing to learn. */
        let cleared = 0;
        for (const t of G.traffic) {
          if (!t.wrecked || t.eaten) continue;
          t.eaten = 1; cleared++;
          for (let k = 0; k < 10; k++) {
            const a = rnd(0, TAU), sp = rnd(90, 320);
            spawn(t.x, t.y, Math.cos(a) * sp, Math.sin(a) * sp,
                  rnd(0.2, 0.5), rnd(3, 7), '176,124,255', true, -5);
          }
        }
        G.pass = 0; G.passT = 0; G.chain = 0; G.mult = 1;
        toast(cleared ? 'PULSE — the road is swept' : 'PULSE', 'red');
        NHAudio.curse();
  ...
  if (G.state === 'play' && !G.ai) {
    ...
    if (d < (b.spec.len + car.spec.len) * 0.38 && car.inv <= 0) {
      ...
      /* was 18-26, sized against a hundred-point bar. Against a five-mistake
         budget that is the entire run in one bump. */
      damage(b.charge > 0 ? 2 : 1, 'boss');
      hitstop(0.06, 24);
      NHAudio.hit(1.2);
    }
  }
...
function stepHazards(dt){
  ...
  for (let i = G.hazards.length - 1; i >= 0; i--) {
    ...
    if (hyp(h.x - car.x, h.y - car.y) < 46) {
      ...
      car.hitFlash = 1;
      /* was 12 — which against a five-mistake budget is not a hazard, it is
         an instant loss. Measured, four of five reader runs ended on a strip
         the moment the Blocker started laying them. */
      damage(1, 'strip');
      NHAudio.hit(0.9);
...
function pursuitEvery(){
  const cleared = (G.run && G.run.cleared) || 0;
  const M = G.run && G.run.M;
  return Math.max(2.8, (8.0 - cleared * 0.8) * ((M && M.wallSpeed) || 1));
}
...
function stepPickups(dt){
  ...
  const want = Math.round(5 * (G.run ? G.run.M.pickupRate : 1));
  ...
  for (let i = G.police.length - 1; i >= 0; i--) {
    ...
    if (lost) {
      ...
      if (gap < 460) {
        ...
        tx = ah.x; ty = ah.y;
        p.blocking = 0.6;
        /* A strip every 1.5s with no cap turned one Blocker into a minefield:
           thirteen strips in twenty seconds, across a road you have to drive.
           Three per blocking stint, spaced, and it has to earn the next one
           by getting in front again. */
        p.strips = p.strips || 0;
        if ((p.dropT = (p.dropT || 0) - dt) <= 0 && p.strips < 3) {
          p.dropT = 3.2; p.strips++; dropHazard(p);
        }
      }
    } else if (A.standoff) {
