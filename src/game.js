...
function step(dt){
  ...
  for (let i = G.police.length - 1; i >= 0; i--) {
    ...
    /* OUTRIDER weaves hard, which is what makes a one-pip unit hard to land */
    if (A.weave) {
      p.weavePh = (p.weavePh || 0) + dt * 3.4;
      st = clamp(st + Math.sin(p.weavePh) * 0.55 * A.weave, -1, 1);
    }
    ...
    p.boostT = Math.max(p.boostT, A.heavy ? 0.05 : 0.2);
    ...
    if (playing && !G.ai) {
      ...
      if (!lost && car.inv <= 0 && d < (p.spec.len + car.spec.len) * 0.36) {
        ...
        if (Ar.ramproof) {
          ...
          hitstop(0.07, 26);
          NHAudio.hit(1.5);
          if (!G.power.shield) damage(1, 'police');
          G.pass = 0; G.passT = 0; G.chain = 0; G.mult = 1;
          car.inv = 0.5;
