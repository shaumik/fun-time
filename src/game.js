...
const LIVERIES = [
  { id:'stock', name:'Factory Cyan', price:0,     col:'#3DE8FF', col2:'#0A5A72', trail:'#FFB13D' },
...
function smash(v, rel){
  ...
  archWreck(v);
  arcFrom(v);
  /* a wreck is the verb; banking is the achievement. See addXP callers. */
  addXP(0.35);
...
function drawTrail(){
  ...
  /* ---- boost reads as hotter, not as a different colour ----
     The ribbon used to shift toward amber under boost, which is where the
     yellow trail came from — and the amber is the better look, so it is the
     stock livery's trail colour now rather than a state you have to be in to
     see. That leaves boost needing its own tell, and shifting an amber trail
     toward amber is no tell at all. A light that is working harder goes
     white, not orange: boost drives it toward a hot near-white instead, which
     reads on every livery rather than only on the cool ones. */
  const col = boost ? mix(base, '#FFF4DA', 0.5) : base;
  ctx.save();
