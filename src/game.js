...
const QF = { bloom:1, wide:1, plate:1, city:1, cityDeep:1, windows:1, glow:1, grain:1, dpr:1.5, tier:'high' };
...
/* ---- the manual override ----
   Every automatic decision in here is a guess about hardware I cannot see,
   and when the guess is wrong the player has no way to say so. This is the
   way to say so: __NH.sharp(2.5) pins the backing store to 2.5 device pixels
   per CSS pixel and takes the tuner out of the resolution business entirely.
   __NH.sharp(0) hands it back. Persisted, so it survives a reload. */
let sharpPin = 0;
try { sharpPin = +(localStorage.getItem("neonheat.sharp") || 0) || 0; } catch (e) {}
...
function applyBackingStore(){
  ...
  const ceiling = sharpPin ? Math.min(sharpPin, native) : Math.min(native, QF.dpr);
  const area = W * H;
  /* MAX_PIXELS is a safety valve against a maximised window on a retina
     desktop asking for eight million pixels a frame. It does not apply to an
     explicit pin: a player who picks Native has said what they want, and a
     control labelled Native that quietly delivers 90% of it is a control that
     lies. The DPR clamp below is still the backstop. */
  const capped = (!sharpPin && area * ceiling * ceiling > MAX_PIXELS)
    ? Math.sqrt(MAX_PIXELS / area)
    : ceiling;
  DPR = clamp(capped * (sharpPin ? 1 : renderScale), 0.4, 3);
...
const REDUCE_MOTION = (window.matchMedia && window.matchMedia('(prefers-reduced-motion:reduce)'))
                      || { matches:false };
/* Live rather than const so a tuning pass can try values in the console
   without a rebuild: __NH.setShake(0.2). */
let SHAKE_MUL = 0.30;
...
function bloom(){
  ...
  /* ---- the blur, by resampling ----
     This was two ctx.filter = 'blur(Npx)' taps, and a CPU profile of twenty
     seconds of real driving put 46.6% of all time in drawImage because of
     them: a Canvas2D filter blur is a per-frame convolution that drops the
     whole composite off the accelerated path, and it was running twice on a
     buffer the size of a quarter of the screen. It is the single reason the
     renderer could not hold full resolution, which is what the player sees
     as the picture being soft.

     A bilinear downscale followed by an upscale is the same operation done
     by the sampler, for free. Halving into bufC and coming back is one wide,
     smooth tap; doing it twice widens the kernel further and still costs
     less than one filtered blit. The look is if anything softer and rounder
     than the 4px/9px pair — a Gaussian is what the two-pass resample
     approximates — and there is no filter left anywhere in the frame. */
  const cw = bufC.width, ch = bufC.height;
  bC.setTransform(1, 0, 0, 1, 0, 0);
  bC.globalCompositeOperation = 'source-over';
  bC.clearRect(0, 0, cw, ch);
  bC.drawImage(bufB, 0, 0, cw, ch);            // down: 1/4 -> 1/8

  bA.clearRect(0, 0, bw, bh);
  bA.drawImage(bufC, 0, 0, bw, bh);            // up: the tight tap

  if (QF.wide) {
    /* a second round-trip for the atmospheric haze: wider still, and added
       into bufA at quarter res so the upscale to the main canvas — the one
       genuinely full-screen blit — happens exactly once */
    bC.clearRect(0, 0, cw, ch);
    bC.drawImage(bufA, 0, 0, cw, ch);
    bA.globalCompositeOperation = 'lighter';
    bA.globalAlpha = 0.55;
    bA.drawImage(bufC, 0, 0, bw, bh);
    bA.globalAlpha = 1;
    bA.globalCompositeOperation = 'source-over';
  }
...
/* ---------------- sharpness ----------------
   The adaptive tuner is a guess about hardware nobody here can see, and when
   it guesses wrong the game is blurry and the player has no way to say so.
   Auto is still the default and still adapts. The two fixed steps take the
   tuner out of the resolution business and render at a share of the display's
   real pixels — Sharp is most of the way to native, Native is all of it. They
   cost frames on a weak device, which is exactly the trade the player is
   being handed rather than having it made for them. */
const SHARP_STEPS = [
  { pin:0,    label:'Sharpness: auto' },
  { pin:2,    label:'Sharpness: sharp' },
  { pin:99,   label:'Sharpness: native' },   // clamped to devicePixelRatio
];
function setSharpLabel(){
  const i = SHARP_STEPS.findIndex(s => s.pin === sharpPin);
  $('btnSharp').textContent = (SHARP_STEPS[i] || SHARP_STEPS[0]).label;
}
$('btnSharp').onclick = e => {
  e.stopPropagation();
  const i = SHARP_STEPS.findIndex(s => s.pin === sharpPin);
  const next = SHARP_STEPS[(i < 0 ? 0 : i + 1) % SHARP_STEPS.length];
  sharpPin = next.pin;
  try { localStorage.setItem('neonheat.sharp', String(sharpPin)); } catch (err) {}
  /* a pinned resolution starts from a clean slate rather than inheriting
     whatever scale the tuner had wound down to */
  renderScale = 1;
  applyBackingStore();
  if (painted) render();
  setSharpLabel(); NHAudio.ui(true);
};
setSharpLabel();
...
/* How much sustained headroom to demand before handing resolution back, and
   whether the last thing the tuner did was hand some back. See the reversal
   note in autoTune: without these the tuner oscillates. The ceiling is low
   enough that the penalty is always recoverable within a run. */
const CLIMB_NEED_BASE = 2500;
const CLIMB_NEED_MAX = 12000;
const CLIMB_STEP = 0.08;
/* What the frame can afford to spend in step + render. The budget is 16.7ms;
   the rest is the browser's — compositing, input, the HUD's own layout. */
const WORK_BUDGET = 13.5;
/* ---- what the tuner is allowed to spend resolution on ----
   These were 17.6 / 19.5 / 28, which is a tuner defending 60fps with the
   player's pixels. On anything fill-limited — a phone, a big window — the
   game dips under 51fps constantly, so it spent resolution until it stopped
   dipping and converged on the one outcome nobody wants: a blurry game
   running at 60.

   For a game you steer with one axis, a steady 45 that is sharp beats a
   blurry 60. So the bad threshold moves out to 23ms and the good one to 20,
   leaving a 3ms dead band, and the picture is what gets defended. A machine
   that can genuinely hold 60 still climbs to the ceiling and stays there —
   nothing here stops it. */
const FRAME_GOOD  = 20;    // under this, headroom to give resolution back
const FRAME_BAD   = 23;    // over this for FRAME_BAD_MS, start giving it up
const FRAME_AWFUL = 34;    // over this, act immediately, do not wait
let climbNeed = CLIMB_NEED_BASE;
let lastWasClimb = false;
...
function autoTune(now, dt){
  /* Headroom was `workAvg < 6.5`, an absolute figure from when the renderer
     ...
     climb back was dead code. Then it was a flat 10.5, which is the same
     mistake one notch along: a constant tuned at one resolution is wrong at
     every other, and at half scale it is far too strict — the picture parks
     low and stays there while the machine holds 60fps with milliseconds a
     frame going spare, which is exactly "why does the quality look worse".

     Predict instead. Cost goes with area, so the next step multiplies the
     work by ((s + STEP) / s)^2 — a third more at half resolution, a tenth
     near the top. Ask whether *that* still fits the budget. The test is then
     correct at every scale, and it stops climbing on its own at the point
     where the next step would not fit rather than at an arbitrary number. */
  const nextCost = workAvg * Math.pow((renderScale + CLIMB_STEP) / renderScale, 2);
  const roomy = ftAvg < FRAME_GOOD && nextCost < WORK_BUDGET;

  /* A hitch is not a verdict. One GC pause, one level-up screen, one ad
     tearing down pushes the average over the line for a few frames — and
     because the drop was instant while the climb needs seconds, treating
     every transient as proof ratchets the picture down over a session and it
     never comes back. Sustained, unless it is genuinely catastrophic. */
  ...
  /* ---- which lever, and in what order ----
     This used to spend resolution all the way to the 0.5 floor before it
     would consider dropping a rung, on the reasoning that resolution is
     "continuous and nearly invisible". The first half is true and the second
     is not: half scale is the single most visible thing in the frame, and it
     is precisely what a player notices and calls the quality getting worse.

     So resolution is the cheap lever only while it is genuinely cheap. Down
     to SOFT_FLOOR the loss really is hard to see and it goes first. Below
     that, a rung of effects — which is now a rung that actually costs
     something, see setQuality — is the better trade, and only once the rungs
     are gone does the floor come off and the last of the resolution go. */
  const SOFT_FLOOR = 0.78;

  if (!sharpPin && missing && renderScale > SOFT_FLOOR) {
    ...
    /* ---- the reversal rule ----
       Arriving here straight after a climb means the climb is what broke the
       budget. A machine whose real cost sits between two scales will then
       oscillate between them forever — measured at 0.77 adjustments a second
       on a mid-range box, and since each adjustment resizes the backing store
       and repaints at a different resolution, the player sees the whole image
       popping about once a second. That, not the cost of any one frame, was
       what read as "the performance got worse".

       So each time a climb has to be undone, demand geometrically more proof
       before trying again. A machine that genuinely has headroom loses
       nothing — it clears the bar on the first try and never comes back
       here. A machine that does not settles within a few reversals and then
       holds one scale, which is what it should have done from the start. */
    ...
    lastWasClimb = false;
    goodMs = 0;
    tuneAt = now + (ftAvg > FRAME_AWFUL ? 320 : 700);
  } else if (missing && tierI > 0) {
    /* Reached only once the branch above has run out of soft floor, so the
       invisible resolution has already been spent and the machine still
       cannot hold the frame. A rung is the right price now; more resolution
       is not. The 2.5s startup grace plus the three resolution steps it takes
       to walk down to the floor give a cold JIT about four and a half seconds
       to settle before anything here is reachable. */
    ...
    demotions++; goodMs = 0;
    tuneAt = now + 1500;
  } else if (!sharpPin && missing && renderScale > SCALE_FLOOR) {
    ...
    goodMs = 0;
    tuneAt = now + 700;
  } else if (!sharpPin && renderScale < 0.99 && goodMs > climbNeed) {
    /* Sustained headroom, not an instantaneous reading. This used to fire on
       ...
       frame and is the other half of the oscillation. goodMs only accumulates
       while roomy holds, so this is "it has been comfortable for climbNeed
       milliseconds without interruption". */
    /* Two climbs in a row means the last one held, so relax the requirement
       again. The reversal penalty has to be forgettable: held forever it is
       just the ratchet by another route, and a machine that had one bad
       minute early would spend the rest of the session refusing to try while
       the player looks at a soft picture on hardware that could carry it. */
    if (lastWasClimb) climbNeed = Math.max(CLIMB_NEED_BASE, climbNeed * 0.5);
    /* A larger step than the old 0.05, because each one now costs a real wait
       rather than a frame: at 0.05 a machine that dipped to the floor needed
       ten steps and twenty-five seconds of sustained headroom to look right
       again. The reversal rule above is what makes the bigger step safe —
       overshoot is paid for once, not forever. */
    renderScale = Math.min(1, renderScale + CLIMB_STEP);
    applyBackingStore();
    if (painted) render();
    lastWasClimb = true;
...
window.__NH = {
  ...
  /* Pin the render resolution in device pixels per CSS pixel, or 0 for auto.
     __NH.sharp() reports what it is actually rendering at right now. */
  sharp(v){
    if (v === undefined) return { pin:sharpPin, native:window.devicePixelRatio || 1,
                                  actual:+DPR.toFixed(2), scale:+renderScale.toFixed(2),
                                  tier:QF.tier, frameMs:+ftAvg.toFixed(1) };
    sharpPin = Math.max(0, +v || 0);
    try { localStorage.setItem("neonheat.sharp", String(sharpPin)); } catch (e) {}
    applyBackingStore();
    if (painted) render();
    return this.sharp();
  },
