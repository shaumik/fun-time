...
function autoTune(now, dt){
  ...
  const roomy = ftAvg < 17.6 && nextCost < WORK_BUDGET;
  ...
  badMs  = ftAvg > 19.5 ? badMs + dt * 1000 : 0;      // below ~51fps
  const missing = badMs > 600 || ftAvg > 28;
  ...
  if (missing && renderScale > SOFT_FLOOR) {
