...
function endRun(won){
  ...
  /* Never a midgame ad on the transition that also offers a rewarded continue.
     CrazyGames allows one or the other between levels, not both: "you can have
     either a midgame ad and restart, or a 'watch rewarded to keep playing',
     but not both". The revive is the continue, so it wins. */
  if (!offeringRevive() && Save.data.runs % 3 === 0) Ads.midroll(showOver);
  else showOver();
}
