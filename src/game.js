...
function newWorld(ai){
  ...
  G.slow = 1; G.flash = 0; G.shake = 0; G.shakePrev = 0; G.dist = 0;
...
/* ---------------- an impact flash with a place ----------------
   G.flash washes the entire canvas, which is right for the things that are
   about the whole run — a unit going down, a bank wiped, the crash. It is
   wrong for a wreck, because a wreck happens somewhere, and by the time you
   own drones, a harpoon, a wrecking ball and cars that cook off a beat after
   they land, plenty of your kills happen behind you or off to one side.

   Washing the screen for one of those is a glow with no cause on screen,
   which reads as a display glitch rather than as feedback — and it is the
   same complaint as the shake was: brightness the player did not ask for and
   cannot attribute. So the world-positioned flashes are gated on being able
   to see the thing that fired them. Off screen, the wreck still pays, still
   sounds, still shakes; it just does not flash a screen it is not in.

   Deliberately not the renderer's onScreen() culler. That one is a generous
   radius — about twice the screen's half-diagonal — because its job is to
   stop cars popping in at the edge, and a wreck it admits can still be a
   whole screen away. This asks the narrower question of whether the player
   could actually see the thing, so it is a screen-space bounds test. The
   margin is small but real: something just past the edge is close enough
   that the eye still reads the flash as belonging to it. */
const FLASH_MARGIN = 90;
/* And at most one of these in any 400ms. A drone kill, a cook-off and a ball
   hit can land within a few frames of each other, and three washes in a third
   of a second is a strobe however faint each one is alone. The loud
   run-level flashes — a unit down, the crash, a bank wiped — do not come
   through here and are never suppressed. */
const FLASH_GAP = 400;
let flashLast = -1e9;
function flashAt(x, y, v){
  const t = performance.now();
  if (t - flashLast < FLASH_GAP) return;
  const [sx, sy] = w2s(x, y);
  const m = FLASH_MARGIN;
  if (sx < -m || sx > W + m || sy < -m || sy > H + m) return;
  flashLast = t;
  G.flash = Math.max(G.flash, v);
}
...
/* How much sustained headroom to demand before handing resolution back, and
   ...
   note in autoTune: without these the tuner oscillates. */
let climbNeed = 2500;
...
function autoTune(now, dt){
  ...
  if (missing && renderScale > 0.5) {
    ...
    if (lastWasClimb) climbNeed = Math.min(climbNeed * 2, 30000);
...
window.__NH = {
  ...
  flashAt,                    // the impact-flash guard, for playtest probes
