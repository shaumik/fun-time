...
/* Three tiers rather than two. High and low alone is a cliff: the step down
   dropped the backing store from 1.5x to 1x and switched off every optional
   pass in the same instant, which is a change you cannot miss. Medium keeps
   the passes that carry the look — the wide bloom tap and the lit windows —
   and gives up the two that are pure garnish, so the common case degrades to
   something that still looks like the game. */
const QF = { bloom:1, wide:1, city:1, windows:1, glow:1, grain:1, ca:1, dpr:1.5, tier:'high' };
const TIERS = ['low', 'medium', 'high'];
function setQuality(t){
  QF.tier = t;
  const high = t === 'high', mid = t === 'medium';
  QF.ca      = high ? 1 : 0;
  QF.grain   = high ? 1 : 0;
  QF.wide    = high || mid ? 1 : 0;
  QF.windows = high || mid ? 1 : 0;
  QF.bloom = 1; QF.city = 1; QF.glow = 1;
  /* the bloom composite is fill-rate bound, so pixels are the lever */
  QF.dpr = high ? 1.5 : mid ? 1.25 : 1;
      ...
      for (let k = 0; k < 3; k++) {
        ...
        spawn(rx, ry, -cs * 260 + rnd(-70, 70), -sn * 260 + rnd(-70, 70),
              /* sized against the streak it sits on: at r=13 the plume was
                 wider than the light it was supposed to be coming out of */
...
/* ============================================================
   ...
   ============================================================ */
/* The reference is a long-exposure photograph of a night road: the streak is
   thin, it runs a long way, and it is continuous. What was here was the
   opposite of all three — 0.55s of it, a hair wider than the car itself, so
   it read as a short fat lozenge bolted to the bumper rather than as light
   left behind. The barrier running alongside it was the giveaway: a 6px core
   inside a soft halo, and that is the thing in frame that actually looks
   expensive.

   So: nearly three times as long, and about a third as wide. */
...
function trailStep(v, dt, cs, sn){
  ...
  /* Sample spacing is chosen so a full-length streak fits inside TRAIL_MAX
     samples. The streak is only a few units wide now, so the faceting a
     coarser interval would show at a hard turn is well under a pixel —
     which is what buys the extra length back at no cost in vertices. */
...
function drawTrail(v, mul, rich){
  ...
  /* Half-width. At 0.52 the streak was as wide as the car; a long-exposure
     streak is a line, so this is the width of a light, not of a vehicle. It
     still opens under drift, because a slide smearing sideways is the one
     time you want to see the width. */
  const base = s.wid * (0.17 + v.drift * 0.13);
  const col = s.accent || s.col;
  const n = t.length - 1;

  /* Nested constant-width bands approximate a lateral falloff. The shape to
     copy is the barrier: a hot filament sitting inside a wide, faint halo,
     with nothing much in between. Weighting it that way — most of the alpha
     in the thinnest band — is what makes a light read as a light rather than
     as a painted stripe. */
  const passes = rich
  ...
  /* Thin bands land under a pixel at the portrait camera, where a sub-pixel
     line does not get thinner, it gets noisier — it flickers as it crosses
     the pixel grid. Floor every width in screen space so the filament stays
     a clean hairline instead of a dotted one. */
  const minHalf = 0.55 / Math.max(cam.zoom, 0.2);

  /* The ribbon is a constant-width wall. Tapering the width toward the tail
     is the obvious way to fade it and it is wrong: a shape that is full width
     at the car and a point behind it is a triangle, and it reads as one — a
     cone stuck to the back of the car rather than a wall being laid down on
     the road. The first cut did it twice over, tapering by position and by
     remaining life, which is why it came out as a dart.
     Width is now flat end to end and the fade is entirely in the alpha. */
  /* Civilian streaks are shortened rather than switched off when the budget
     tightens. Twenty-six of them at full length is a lot of additive fill,
     but cutting them entirely breaks the effect — a long exposure where only
     the hero cars smear reads as a bug. Half the length keeps every light in
     the frame streaking at a fraction of the area. */
  ...
  for (const [wm, c, al] of passes) {
    ...
    for (let i = from; i <= n; i++) {
      const p = t[i];
      if (i === from) ctx.moveTo(p.x + p.nx * w, p.y + p.ny * w);
    ...
    ctx.closePath();
    /* tail to head, so the wall dissolves into the dark behind the car
...
/* Headlight beams were a trapezoid whose gradient faded along its length
   only, which leaves both long sides as razor-hard straight edges. Seen from
   directly overhead that is not a beam of light, it is a grey cardboard
   wedge — the one shape in the frame that was neither emissive nor soft, and
   the thing the eye picks out as fake before it notices anything else.

   Baked once as a real beam instead: alpha falls off across the cone as well
   as along it, so it has shoulders rather than edges, and the cone opens with
   distance. Per-pixel is fine for something built one time and then blitted. */
let beamSprite = null;
function makeBeamSprite(){
  ...
  for (let y = 0; y < N; y++) {
    for (let x = 0; x < N; x++) {
      ...
      const v = (y / (N - 1) - 0.5) * 2;        // across it
      const hw = 0.22 + 0.78 * u;               // the cone opens with distance
      const t = Math.min(1, Math.abs(v) / hw);
...
function drawVehicle(v, opt){
  ...
  if (o.glow !== false) {
    ctx.globalCompositeOperation = 'lighter';
    /* Hugging the chassis, not ballooning around it. At len*1.05 by wid*1.7
       this was a glowing lozenge half again the size of the car, sitting
       right where the streak leaves the bumper — the two read as one fat
       blob with a tail. */
    blitGlow(em, 0, 0, s.len * 0.72, s.wid * 1.0, v.kind === 'player' ? 0.30 : 0.20);
    ctx.globalCompositeOperation = 'source-over';
  }
...
function render(){
  ...
  /* Every ribbon before every car, so a trail never paints over a chassis.
     Civilian streaks used to be gated to the high tier, on the grounds that
     nobody looks at them. That was wrong once the look became long exposure:
     a night-road photograph is every light streaking, and a frame where only
     the hero cars smear and the traffic sits still reads as broken rather
     than as cheap. At a third of the old width they cost a fraction of what
     they did when the gate was written, so they stay on everywhere. */
...
let demotions = 0;    // how many times we have had to give up a tier
...
function autoTune(now, dt){
  ...
  /* Headroom was `workAvg < 6.5`, an absolute figure from when the renderer
     was cheaper. Nothing about the scene hits 6.5ms any more, so the test
     could never pass and the climb back up was dead code — a session that
     dipped once stayed dipped, holding a rock-solid 60fps at half resolution
     with six milliseconds a frame going spare. The budget is 16.7ms; spending
     under two thirds of it is what "roomy" means. */
  ...
  if (missing && renderScale > 0.5) {
    ...
    /* out of resolution to give: step down one tier, not straight to the floor */
    ...
    /* Give a tier back, but only after real sustained headroom, and demand
