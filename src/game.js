...
/* ---------------- gesture steering ----------------
   ...
   nothing to aim at. */
const GS = {
  on:false, id:-1, ax:0, ay:0, x:0, y:0, steer:0
};
const gsMaxX = () => clamp(W * 0.13, 38, 120);   // travel to full lock

function gsReset(){ GS.on = false; GS.id = -1; GS.steer = 0; }
function gsDown(e){
...
function gsUp(e){
  ...
  GS.on = false; GS.id = -1; GS.steer = 0;
}
...
class Vehicle {
  ...
  drive(dt, steerIn, throttle){
    ...
    let acc = throttle * s.power * (this.boost ? 1.9 : 1);
...
function chainTime(){
  return Math.max(1.4, (G.run ? G.run.M.chainTime : 3.2));
}
...
/* ============================================================
   ...
   ============================================================ */
const POWERS = [
  { id:'boost',  name:'Boost',     col:'#FFB13D', rgb:'255,177,61',  glyph:'\u00bb', w:34 },
  { id:'repair', name:'Repair',    col:'#2FE08A', rgb:'47,224,138',  glyph:'+',       w:22 },
  { id:'shield', name:'Ram Plate', col:'#3DE8FF', rgb:'61,232,255',  glyph:'\u25c7', w:20 },
  { id:'surge',  name:'Surge',     col:'#FF2E88', rgb:'255,46,136',  glyph:'\u2605', w:16 }
];
...
function takePickup(k){
  ...
  toast(k.name + '!', k.id === 'repair' ? 'gold' : k.id === 'surge' ? 'pink' : '');
  for (let i = 0; i < 22; i++) {
...
function syncPowers(){
  ...
  for (let i = 0; i < list.length; i++) {
    if (nodes[i]) nodes[i].lastChild.textContent = list[i][2].toFixed(1) + 's';
  }
}
...
$('btnPlay').onclick   = () => { NHAudio.resume(); toGarage(); };
